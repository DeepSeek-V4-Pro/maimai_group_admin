...
class CommandMixin:
    ...
    async def cmd_admin_undo(self, stream_id: str = "", user_id: str = "", matched_groups: dict | None = None, **kwargs: Any):
        ...
        if not isinstance(matched, dict) or not matched.get("qq"):
            fragment = raw_text.split("undo", 1)[-1] if "undo" in raw_text else raw_text
            for m in re.finditer(r"@(\d+)", fragment):
                matched = {"qq": m.group(1)}; break
            if not matched.get("qq"):
        ...
        if not ok:
            await self.ctx.send.text("解禁未能生效，请检查权限", stream_id)
            return True, "", True
        removed = False
        async with self._lock:
            ...
            if gid_str in exempt and str(qq) in exempt[gid_str]:
                ...
                removed = True
        suffix = "，同时移出豁免名单" if removed else ""
        await self._send_at_text(stream_id, "已强制解禁", qq, suffix)
        return True, "", True
    ...
    async def cmd_admin_ban(self, stream_id: str = "", user_id: str = "", matched_groups: dict | None = None, **kwargs: Any):
        ...
        raw_text = str(kwargs.get("text", ""))
        if not isinstance(matched, dict) or not matched.get("qq"):
            fragment = raw_text.split("ban", 1)[-1] if "ban" in raw_text else raw_text
            for m in re.finditer(r"@(\d+)", fragment):
                matched = {"qq": m.group(1)}; break
            if not matched.get("qq"):
                m = re.search(r"(\d+)", fragment)
    ...
    async def cmd_admin_unban(self, stream_id: str = "", user_id: str = "", matched_groups: dict | None = None, **kwargs: Any):
        ...
        raw_text = str(kwargs.get("text", ""))
        if not isinstance(matched, dict) or not matched.get("qq"):
            fragment = raw_text.split("unban", 1)[-1] if "unban" in raw_text else raw_text
            for m in re.finditer(r"@(\d+)", fragment):
                matched = {"qq": m.group(1)}; break
            if not matched.get("qq"):
                m = re.search(r"(\d+)", fragment)
    ...
    async def cmd_admin_shutlist(self, stream_id: str = "", user_id: str = "", **kwargs: Any):
        ...
        if ok and isinstance(data, dict):
            ...
            if isinstance(shut_list, list) and shut_list:
                lines = [f"群 {gid} 禁言列表 ({len(shut_list)} 人):"]
