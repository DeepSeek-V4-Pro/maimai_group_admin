...
class PluginCore(MaiBotPlugin):
    ...
    async def _save_exempt_users(self):
        try:
            ...
            exempt_table = config_data["safeguard"].get("exempt_users")
            if exempt_table is None or not isinstance(exempt_table, dict):
                exempt_table = tomlkit.table()
                config_data["safeguard"]["exempt_users"] = exempt_table
            for key in list(exempt_table.keys()):
                del exempt_table[key]
    ...
    async def _save_enabled_groups(self):
        try:
            ...
            existing = config_data["auto_moderate"].get("enabled_groups")
            if existing is None or not hasattr(existing, "clear"):
                existing = tomlkit.array()
                config_data["auto_moderate"]["enabled_groups"] = existing
            existing.clear()
