...
| `mode` | string | `"inline"` | 审核模式：`"inline"`=在当前消息中注入，`"separate"`=独立消息 **（暂未实现）**，需配合 throttle_seconds |
| `throttle_seconds` | int | `5` | separate 模式下两次注入的最小间隔（秒）**（暂未实现）** |
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `public_notify` | bool | `true` | 是否在群内公开通知管理操作 **（暂未实现，当前固定发送简版通知）** |
| `public_style` | string | `"playful"` | 通知风格 **（暂未实现）** |
| `admin_dm_on_severe` | bool | `true` | 严重操作私聊通知管理员 **（暂未实现）** |
...
| `group_actions` | dict[str,str] | `{}` | 按群覆盖 default_action: `"群号" = "approve"/"reject"/"ignore"` |
| `group_reject_keywords` | dict[str,list] | `{}` | 按群覆盖拒绝关键词: `"群号" = ["广告","推广"]` |
| `group_require_keywords` | dict[str,list] | `{}` | 按群覆盖必须关键词 |
| `group_limits` | dict[str,list] | `{}` | 按群覆盖每日限额: `"群号" = [approve_limit, reject_limit]`，0 使用全局值 |

> 配置示例：
> ```toml
> [auto_approve.group_actions]
> "123456789" = "approve"
> 
> [auto_approve.group_reject_keywords]
> "123456789" = ["广告", "推广"]
> 
> [auto_approve.group_limits]
> "123456789" = [5, 10]
> ```
...
| `log_skip_decisions` | bool | `false` | 是否记录 LLM 决定"跳过/不操作"的决策 **（暂未实现）** |
...
| `mute_response_style` | string | `"活泼俏皮..."` | 禁言后发言风格 **（暂未实现，LLM 按人设自行发挥）** |
| `kick_response_style` | string | `"严肃但不凶..."` | 踢人后发言风格 **（暂未实现，LLM 按人设自行发挥）** |
| `essence_response_style` | string | `"简洁夸奖..."` | 设精后发言风格 **（暂未实现，LLM 按人设自行发挥）** |
| `admin_notify_template` | string | (模板文本) | 严重操作私聊通知模板 **（暂未实现）** |
...
2. 检测该群是否有 `group_actions` 覆盖，有则使用覆盖值，否则用全局 `default_action`
3. 同样按群检查 `group_reject_keywords`、`group_require_keywords`、`group_limits`
4. 超过 `max_pending_seconds` 的申请自动跳过
5. 检查 `reject_keywords`：命中 → 拒绝
6. 检查 `require_message_keywords`：未全部满足 → 忽略
7. 执行 approve/reject，受限额约束
...
**解决**：设置全局 `enabled = true` 并 `default_action = "approve"/"reject"`，或在 `group_actions` 中为指定群覆盖
