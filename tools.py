...
class ToolMixin:
    ...
    async def tool_mute_user(self, group_id: int = 0, user_id: int = 0, duration: int = 0, reason: str = "", **kwargs: Any) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            try:
                ok, data = await self._call_api(api_name="adapter.napcat.group.set_group_ban", group_id=group_id, user_id=user_id, duration=duration)
    ...
    async def tool_post_notice(self, group_id: int = 0, content: str = "", **kwargs: Any) -> dict[str, Any]:
        ...
        async with self._lock:
            try:
                ok, data = await self._call_action_api(api_name="adapter.napcat.group.send_group_notice", group_id=group_id, content=content)
    ...
    async def tool_get_notice(self, group_id: int = 0, **kwargs: Any) -> dict[str, Any]:
        ...
        async with self._lock:
            try:
                ok, data = await self._call_action_api(api_name="adapter.napcat.group.get_group_notice", group_id=group_id)
                if ok and isinstance(data, dict):
    ...
    async def tool_get_system_msg(self, group_id: int = 0, **kwargs: Any) -> dict[str, Any]:
        ...
        async with self._lock:
            try:
                ok, data = await self._call_action_api(api_name="adapter.napcat.group.get_group_system_msg", group_id=group_id)
                now = datetime.now()
                if ok and isinstance(data, dict):
